...
class CapaianController extends Controller
{
    ...
    public function exportPDF(Request $request)
    {
        ...
        $dompdf->setPaper('A4', 'landscape'); // Landscape karena tabel lebar
        $dompdf->render();

        $filterText = $cplIdTerpilih ? '_' . Cpl::find($cplIdTerpilih)->kodeCpl : '_Semua_CPL';
        $fileName = 'Capaian_CPL_CPMK_' . str_replace(' ', '_', $mahasiswa->nama ?? 'Mahasiswa') . $filterText . '_' . date('Y-m-d') . '.pdf';
