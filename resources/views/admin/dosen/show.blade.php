@extends('layouts.main')

@section('title', 'Detail Dosen')

@section('content')
<div class="p-4 md:p-6 space-y-4">
  

    <div class="bg-white border border-gray-200 rounded-xl">
        <div class="p-6 border-b border-gray-200 flex justify-between items-center">
            <h2 class="text-lg font-semibold text-gray-900">Detail Dosen</h2>
            <div class="flex space-x-2">
                <a href="{{ route('admin.dosen.edit', $dosen->id) }}" 
                   class="bg-amber-600 hover:bg-amber-700 text-white px-4 py-2 rounded-lg flex items-center">
                    <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path>
                    </svg>
                    Edit
                </a>
                <a href="{{ route('admin.dosen.index') }}" 
                   class="bg-gray-600 hover:bg-gray-700 text-white px-4 py-2 rounded-lg flex items-center">
                    <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"></path>
                    </svg>
                    Kembali
                </a>
            </div>
        </div>
        
        <div class="p-6">
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
                <div>
                    <h3 class="text-lg font-semibold text-gray-900 mb-4">Informasi Pribadi</h3>
                    <div class="space-y-4">
                        <div class="flex justify-between py-2 border-b border-gray-200">
                            <span class="font-medium text-gray-700">NIP:</span>
                            <span class="text-gray-900">{{ $dosen->nip }}</span>
                        </div>
                        <div class="flex justify-between py-2 border-b border-gray-200">
                            <span class="font-medium text-gray-700">Nama Lengkap:</span>
                            <span class="text-gray-900">{{ $dosen->nama }}</span>
                        </div>
                        <div class="flex justify-between py-2 border-b border-gray-200">
                            <span class="font-medium text-gray-700">Email:</span>
                            <span class="text-gray-900">{{ $dosen->user->email }}</span>
                        </div>
                        <div class="flex justify-between py-2 border-b border-gray-200">
                            <span class="font-medium text-gray-700">Status:</span>
                            <span>
                                @if($dosen->user->isAktif)
                                    <span class="text-sm font-medium text-emerald-700">Aktif</span>
                                @else
                                    <span class="text-sm font-medium text-red-700">Tidak Aktif</span>
                                @endif
                            </span>
                        </div>
                        <div class="flex justify-between py-2 border-b border-gray-200">
                            <span class="font-medium text-gray-700">Role:</span>
                            <span class="inline-flex px-2 py-0.5 text-xs font-medium rounded bg-gray-100 text-gray-700">{{ ucfirst($dosen->user->role) }}</span>
                        </div>
                        <div class="flex justify-between py-2 border-b border-gray-200">
                            <span class="font-medium text-gray-700">Tanggal Dibuat:</span>
                            <span class="text-gray-900">{{ $dosen->created_at->format('d/m/Y H:i') }}</span>
                        </div>
                        <div class="flex justify-between py-2 border-b border-gray-200">
                            <span class="font-medium text-gray-700">Terakhir Diupdate:</span>
                            <span class="text-gray-900">{{ $dosen->updated_at->format('d/m/Y H:i') }}</span>
                        </div>
                    </div>
                </div>
                
                <div>
                    <h3 class="text-lg font-semibold text-gray-900 mb-4">Informasi Akun</h3>
                    <div class="bg-gray-50 border border-gray-200 rounded-xl p-4">
                        <div class="space-y-3">
                            <div class="flex justify-between">
                                <span class="font-medium text-gray-700">User ID:</span>
                                <span class="text-gray-900">{{ $dosen->user->id }}</span>
                            </div>
                            <div class="flex justify-between">
                                <span class="font-medium text-gray-700">Status Akun:</span>
                                <span class="text-gray-900">{{ ucfirst($dosen->user->status) }}</span>
                            </div>
                            <div class="flex justify-between">
                                <span class="font-medium text-gray-700">Email Verified:</span>
                                <span>
                                    @if($dosen->user->email_verified_at)
                                        <span class="text-sm font-medium text-emerald-700">Terverifikasi</span>
                                    @else
                                        <span class="text-sm font-medium text-amber-700">Belum Terverifikasi</span>
                                    @endif
                                </span>
                            </div>
                            @if($dosen->user->email_verified_at)
                                <div class="flex justify-between">
                                    <span class="font-medium text-gray-700">Tanggal Verifikasi:</span>
                                    <span class="text-gray-900">{{ $dosen->user->email_verified_at->format('d/m/Y H:i') }}</span>
                                </div>
                            @endif
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</div>
@endsection 