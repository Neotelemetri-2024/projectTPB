    ...
    <div class="bg-white border border-gray-200 rounded-xl">
        <div class="p-6 border-b border-gray-200">
            <div class="flex items-center justify-between">
                <h2 class="text-lg font-semibold text-gray-900">Tambah Kurikulum</h2>
        ...
        <form action="{{ route('admin.kurikulum.store') }}" method="POST" class="p-6">
            @csrf
            <div class="grid gap-4 mb-4 grid-cols-1 md:grid-cols-2">
                <div>
                    ...
                    <input type="text" name="kode" id="kode" value="{{ old('kode') }}"
                           class="bg-gray-50 border {{ $errors->has('kode') ? 'border-red-500' : 'border-gray-300' }} text-gray-900 text-sm rounded-lg focus:ring-amber-500 focus:border-amber-500 block w-full p-2.5"
                           placeholder="2020" required>
                    <p class="mt-1 text-xs text-gray-500">Kode ini yang dipakai di kolom KURIKULUM saat import mata kuliah.</p>
                ...
                <div>
                    ...
                    <input type="text" name="nama" id="nama" value="{{ old('nama') }}"
                           class="bg-gray-50 border {{ $errors->has('nama') ? 'border-red-500' : 'border-gray-300' }} text-gray-900 text-sm rounded-lg focus:ring-amber-500 focus:border-amber-500 block w-full p-2.5"
                           placeholder="Kurikulum 2020" required>
                    @error('nama')<p class="mt-1 text-sm text-red-600">{{ $message }}</p>@enderror
                </div>
                <div>
                    <label for="tahun" class="block mb-2 text-sm font-medium text-gray-900">Tahun</label>
                    <input type="number" name="tahun" id="tahun" value="{{ old('tahun') }}" min="1900" max="2100"
                ...
                <div>
                    ...
                    <select name="isAktif" id="isAktif" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-amber-500 focus:border-amber-500 block w-full p-2.5">
                        ...
                        <option value="0" {{ old('isAktif') === '0' ? 'selected' : '' }}>Nonaktif</option>
                    </select>
                </div>
                <div class="md:col-span-2">
                    <label for="deskripsi" class="block mb-2 text-sm font-medium text-gray-900">Deskripsi</label>
                    <textarea name="deskripsi" id="deskripsi" rows="4"
                              class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-amber-500 focus:border-amber-500 block w-full p-2.5 resize-none"
                              placeholder="Deskripsi kurikulum (opsional)">{{ old('deskripsi') }}</textarea>
                </div>
            </div>
            ...
            <div class="flex items-center justify-end space-x-2">
                ...
                <button type="submit" class="text-white bg-amber-600 hover:bg-amber-700 rounded-lg text-sm font-medium px-5 py-2.5">Simpan Kurikulum</button>
            </div>
        </form>
    </div>
</div>
