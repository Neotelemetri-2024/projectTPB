    ...
    <div class="bg-white border border-gray-200 rounded-xl overflow-hidden">
        <div class="px-5 py-4 border-b border-gray-200">
            <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                <div>
                    <h1 class="text-xl font-semibold text-gray-900 tracking-tight">Kelola Mahasiswa</h1>
        ...
        @if(session('success'))
            <div class="mx-5 mt-4 bg-emerald-50 border border-emerald-200 text-emerald-700 px-4 py-3 rounded-md" role="alert">
                <span class="block sm:inline">{{ session('success') }}</span>
            </div>
        @endif

        @if($errors->any())
            <div class="mx-5 mt-4 bg-red-50 border border-red-200 text-red-700 px-4 py-3 rounded-md" role="alert">
            ...
            <form method="GET" action="{{ route('admin.tahun-ajaran-matkul.manage-mahasiswa', $tahunAjaranMatkul->id) }}">
                <div class="flex flex-col md:flex-row gap-3 items-end">
                    <div class="flex-shrink-0 w-full md:w-48">
                        ...
                        <select name="tahun_masuk" class="border border-gray-300 text-gray-900 text-sm rounded-md focus:ring-amber-500 focus:border-amber-500 block w-full p-2" onchange="this.form.submit()">
                    ...
                    <div class="flex-1 w-full">
                        <label class="block text-[11px] font-medium text-gray-500 mb-1">Cari Mahasiswa (Nama/NIM)</label>
                        <input type="text" name="search" class="border border-gray-300 text-gray-900 text-sm rounded-md focus:ring-amber-500 focus:border-amber-500 block w-full p-2" placeholder="Cari nama atau NIM mahasiswa..." value="{{ request('search') }}">
                    </div>
                    <div class="flex-shrink-0 flex gap-2">
                        ...
                        <a href="{{ route('admin.tahun-ajaran-matkul.manage-mahasiswa', $tahunAjaranMatkul->id) }}" class="bg-gray-600 hover:bg-gray-700 text-white px-3 py-2 rounded-md flex items-center text-sm">
                            <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="px-5 py-3 border-b border-gray-200 bg-gray-50">
                <div class="flex items-center justify-between gap-3">
                    ...
                    <button type="button"
                            ...
                            class="bg-green-600 hover:bg-green-700 text-white px-3 py-2 rounded-md flex items-center text-sm" id="addSelectedBtn" disabled>
                        <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    <thead class="bg-gray-50">
                        <tr>
                            ...
                            <th class="px-5 py-3 text-left text-[11px] font-semibold uppercase tracking-wide text-gray-500">NIM</th>
                            <th class="px-5 py-3 text-left text-[11px] font-semibold uppercase tracking-wide text-gray-500">Nama</th>
                            <th class="px-5 py-3 text-left text-[11px] font-semibold uppercase tracking-wide text-gray-500">Tahun Masuk</th>
                            <th class="px-5 py-3 text-left text-[11px] font-semibold uppercase tracking-wide text-gray-500">Email</th>
                        </tr>
                    </thead>
                    <tbody class="bg-white divide-y divide-gray-200">
                        @forelse($availableMahasiswas as $mahasiswa)
                            <tr class="hover:bg-gray-50 mahasiswa-row">
                                ...
                                <td class="px-5 py-3 whitespace-nowrap text-sm font-medium text-gray-900">{{ $mahasiswa->nim }}</td>
                                <td class="px-5 py-3 whitespace-nowrap text-sm text-gray-900">{{ $mahasiswa->nama }}</td>
                                <td class="px-5 py-3 whitespace-nowrap text-sm text-gray-700">
                                    {{ $mahasiswa->tahunMasuk }}
                                </td>
                                <td class="px-5 py-3 whitespace-nowrap text-sm text-gray-900">{{ $mahasiswa->user->email ?? '-' }}</td>
                            </tr>
                        @empty
                            <tr>
