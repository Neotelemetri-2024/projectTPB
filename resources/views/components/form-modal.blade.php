        ...
        <div class="relative bg-white rounded-lg shadow-xl dark:bg-gray-700 animate-in fade-in slide-in-from-top-4 duration-300">
            ...
            <form action="{{ $action }}" method="POST" class="p-4 md:p-5" {!! isset($enctype) ? 'enctype="' . $enctype . '"' : '' !!}>
                ...
                <div class="flex items-center justify-end space-x-3 pt-4 border-t border-gray-200 dark:border-gray-600">
                    ...
                    <button type="submit" class="text-white bg-amber-600 hover:bg-amber-700 focus:ring-4 focus:outline-none focus:ring-amber-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-amber-600 dark:hover:bg-amber-700 dark:focus:ring-amber-800 transition-colors duration-200">
                        {{ $submitText ?? 'Simpan' }}
                    </button>
                </div>
            </form>
