...
<div class="p-6">
    ...
    <nav class="mb-6">
        <ol class="flex items-center space-x-2 text-sm text-gray-500">
            <li>
                <a href="{{ route('pimpinan.dashboard') }}" class="hover:text-gray-700">Dashboard</a>
            </li>
            <li>
            ...
            <li>
                <a href="{{ route('pimpinan.cpmk-report.index') }}" class="hover:text-gray-700">Laporan CPMK</a>
            </li>
            <li>
                <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
                    <path fill-rule="evenodd" d="M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z" clip-rule="evenodd"></path>
            ...
            <div class="bg-white rounded-lg shadow-md mb-6">
                <div class="p-6 border-b border-gray-200">
                    ...
                    <div class="flex items-center gap-4 mt-2 text-sm text-gray-600">
                        <span class="bg-blue-100 text-blue-800 px-2 py-1 rounded">Total: {{ $data['total_mahasiswa'] }} Mahasiswa</span>
                        <span class="bg-green-100 text-green-800 px-2 py-1 rounded">Dengan Nilai: {{ $data['mahasiswa_dengan_nilai'] }} Mahasiswa</span>
                        <span class="bg-amber-100 text-amber-800 px-2 py-1 rounded">Rata-rata: {{ $data['average_nilai'] }}</span>
                        <span class="bg-purple-100 text-purple-800 px-2 py-1 rounded">Kompeten: {{ $data['competent_percentage'] }}%</span>
                    </div>
                </div>
                        ...
                        <table class="min-w-full bg-white border border-gray-200 rounded-lg">
                            ...
                            <tbody class="divide-y divide-gray-200">
                                <tr>
                                    ...
                                    <td class="px-4 py-3 text-sm text-gray-900 border-r border-gray-200" rowspan="4">{{ $data['competent_percentage'] }}%</td>
                                    <td class="px-4 py-3 text-sm text-gray-900" rowspan="4">{{ $data['not_competent_percentage'] }}%</td>
                                </tr>
                                <tr>
...
chartData.forEach((data, index) => {
    ...
    if (histogramCtx) {
        const histogramChart = new Chart(histogramCtx, {
            type: 'line',
            data: {
                labels: data.histogram_data.map(item => item.range),
                datasets: [
                    {
                        label: 'Frekuensi',
                        data: data.histogram_data.map(item => item.count),
                        borderColor: '#F97316',
                        borderWidth: 3,
            ...
            options: {
                ...
                scales: {
                    y: {
                        ...
                        max: Math.max(...data.histogram_data.map(item => item.count)) + 5
                    },
                    x: {
                        title: {
                            display: true,
